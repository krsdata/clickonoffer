    ...
    <fieldset class="step ui-formwizard-content" id="step1" style="display: block;">
        ...
        <div class="row">
			 <div class="col-md-6">
                 <div class="form-group {{ $errors->first('parent_cat_id', ' has-error') }}">
                    <label>Category Group: <span class="text-danger">*</span></label>
					<select class="form-control required" name="parent_cat_id">
                     @foreach($parentCat as $key => $result) 
						<option value="{{$result->id}}">{{$result->name}}</option>
					 @endforeach					 
					 </select>
                     <span class="help-block">{{ $errors->first('parent_cat_id', ':message') }}</span>
                </div>
            </div>
            <div class="col-md-6">
